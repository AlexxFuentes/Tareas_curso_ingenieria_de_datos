...
SELECT
	s.tex_name AS Deporte,
    m.tex_name As Metrica,
    sum(w.dec_calories_burned) AS total_calorias
FROM
    Sport AS s
INNER JOIN 
	SportMetric AS sm
    ON s.id = sm.id_sport_fk
INNER JOIN 
	Metric AS m
    ON m.id = sm.id_metric_fk
INNER JOIN 
	Workout AS w
    ON w.id = sm.id_workout_fk 
GROUP BY m.tex_name, s.tex_name
...
INNER JOIN 
	Metric AS m
    ON m.id = sm.id_metric_fk
WHERE s.tex_name = "Boxing" AND m.tex_name = "intensity punches" GROUP BY sm.dec_value;
...
SELECT
	s.tex_name,
    -- w.dec_avg_heart_rate,
    max(w.dec_avg_heart_rate)
FROM
 Sport AS s
INNER JOIN 
	SportMetric AS sm
...
INNER JOIN 
	Workout AS w
    ON w.id = sm.id_workout_fk
WHERE s.tex_name = "Cycling" OR s.tex_name = "Running" GROUP BY s.tex_name;
